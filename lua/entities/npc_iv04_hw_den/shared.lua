...
ENT.Models  = {"models/halowars1/flood/den.mdl"}
...
function ENT:SpawnSquad(sq)
	...
	for i = 1, tims do
		...
		ent:SetPos(p+ag:Forward()*(40*i)+self:GetUp()*60)
		ent:SetAngles(ag)
		ent:Spawn()
		ent:SetOwner(self)
...
function ENT:CreateRagdoll(dmg)
	...
	if !self.DoFade then
		...
		timer.Simple( fr, function()
			if IsValid(en) then
				imbackbitch = ents.Create("npc_iv04_hw_den")
...
list.Set( "NPC", "npc_iv04_hw_den", {
	Name = "Flood Den",
	Class = "npc_iv04_hw_den",
